...
fn printer_thread(mut printer_consumer: UnixStream) {
    println!("Printer thread started");
    loop {
        let mut length_buffer = [0u8; 1];
        printer_consumer.read_exact(&mut length_buffer).unwrap();
        
        let length: usize = std::str::from_utf8(&length_buffer).unwrap().parse().unwrap();

        let mut input_buffer = vec![0u8; length];
        printer_consumer.read_exact(&mut input_buffer).unwrap();
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let printer_handle = thread::spawn(|| { printer_thread(printer_consumer)});
